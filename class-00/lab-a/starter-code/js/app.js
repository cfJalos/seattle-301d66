...
function displayPics(){
  ...
  // const only existed in line 32-33. it's blocked scope line 35 doesnt have access to
  console.log(viewed);
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(event.target.id.concat(' has ',allProducts[i].votes,' votes in ',allProducts[i].views,' views'));
    }
  }
...
function showList() {
  for(let i = 0; i < allProducts.length; i++) {
    ...
    liEl.textContent = allProducts[i].name.concat(' has ',allProducts[i].votes,' votes in ',allProducts[i].views,' views');
